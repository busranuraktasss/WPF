...
namespace WindowMove
{
    ...
    public class WindowMove : Application
    {
        ...
        public void btnMove_Click(object src, RoutedEventArgs e)
        {
            Creator creator = new Creator();
            var x1 = double.Parse(_xCoordinate.Text);
            var y1 = double.Parse(_yCoordinate.Text);

            if (x1>300)
            {
                Mesaj MesajA = creator.FactoryMethod(AllMessage.MesajA);
                MesajA.Send();

            }
            if(y1>300)
            {
            Mesaj MesajB = creator.FactoryMethod(AllMessage.MesajB);
            MesajB.Send();
            }
